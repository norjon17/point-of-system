...
export default function SalesCOActions({ posForm }: IProps) {
  ...
  const [customerOrder, setCustomerOrder] = useState<POSFormType | null>(null)
  ...
  const summary = useMemo(() => {
    const price = ordersWatch.reduce(
      (sum, item) =>
        ...
        (item.selling_price * item.quantity -
          (item.discount && !isNaN(item.discount) ? item.discount : 0)),
      0
    )
    ...
    const sub_total =
      ...
      service_fee +
      delivery_fee -
      (discountFee && !isNaN(discountFee) ? Math.abs(Number(discountFee)) : 0)

    const vat = Number((sub_total * 0.12).toFixed(2))
    const balance = sub_total + vat
        ...
        <Stack>
          ...
          <Table flex={1} withRowBorders={false}>
            <Table.Tbody>
              ...
              <Table.Tr>
                ...
                <Table.Td>
                  <Text style={{ textAlign: "right" }}>
                    {formatNumberWithCommas(summary.sub_total)}
                  </Text>
                </Table.Td>
              </Table.Tr>
              <Table.Tr>
                ...
                <Table.Td>
                  <Text style={{ textAlign: "right" }}>{summary.vat}</Text>
                </Table.Td>
              </Table.Tr>
              <Table.Tr>
                ...
                <Table.Td fw={600} fz={18} style={{ textAlign: "right" }}>
          ...
          {ordersWatch.length > 0 && (
            <Group justify="flex-end">
              ...
              <Button onClick={() => posForm.handleSubmit(onSubmit)()}>
                Checkout
              </Button>
            </Group>
          )}
      ...
      >
        <Stack>
          ...
          {customerOrder?.cash || change ? (
            <Group>
              <Text fw={600} fz={18}>
                {customerOrder?.cash && customerOrder.cash > 0 && change >= 0
                  ? "Change (₱):"
                  : ""}
                {!customerOrder?.cash || customerOrder.cash === 0 || change < 0
                  ? "Receivable Amount (₱):"
          ...
          <TextInput
            ...
            onChange={(e) => {
              ...
              if (cash) {
                setChange(cash - summary.balance)
